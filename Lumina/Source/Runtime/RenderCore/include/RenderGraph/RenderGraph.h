...
public:
    ...
    [[nodiscard]] FRenderGraph* GetRenderGraph() const { return mpGraph; }
...
public:
    ...
    TPass* AddPassObject(TArgs&&... Args)
    {
        auto* pPass = new TPass(std::forward<TArgs>(Args)...);

        auto PassIndex = static_cast<uint32_t>(mPasses.size());
        FPass InternalPass;
        InternalPass.Name = pPass->GetName();
        mPasses.push_back(std::move(InternalPass));

        FRenderGraphPassBuilder Builder(this, PassIndex);
        pPass->Setup(Builder);
        mPassObjects.push_back(std::unique_ptr<FRenderPassBase>(pPass));
...
private:
    ...
    std::vector<std::unique_ptr<FRenderPassBase>> mPassObjects;
};