#include "Renderer/D3D12/D3D12ShaderCompiler.h"
...
bool ShaderUtils::FD3D12Blob::IsNull() const
{
    return !pD3DBlob && !pDxcBlob;
}

const void* ShaderUtils::FD3D12Blob::GetByteCode() const
{
    if (this->pD3DBlob)
...
size_t ShaderUtils::FD3D12Blob::GetByteCodeSize() const
{
    if (this->pD3DBlob)
...
ShaderUtils::FD3D12Blob ShaderUtils::CompileFromSource(const FD3D12ShaderStageCompileDesc& ShaderStageCompileDesc,
...
bool ShaderUtils::CompileFromCachedBinary(const std::string& ShaderBinaryFilePath, FD3D12Blob& Blob, bool bSM6,
    std::string& ErrorMsg)
{
    return false;
}
