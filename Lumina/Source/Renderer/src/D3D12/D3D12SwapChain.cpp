#include "Renderer/D3D12/D3D12SwapChain.h"
...
bool FD3D12SwapChain::Create(const FSwapChainCreateDesc& Desc)
{
...
HRESULT FD3D12SwapChain::Resize(int Width, int Height, DXGI_FORMAT Format)
{
...
HRESULT FD3D12SwapChain::Present()
{
...
void FD3D12SwapChain::MoveToNextFrame()
{
...
void FD3D12SwapChain::WaitForGPU()
{
    if (mpPresentQueue)
        mpPresentQueue->Flush();
}

void FD3D12SwapChain::CreateRenderTargetViews()
{
    mRenderTargets.resize(mNumBackBuffers);
...
void FD3D12SwapChain::DestroyRenderTargetViews()
{
