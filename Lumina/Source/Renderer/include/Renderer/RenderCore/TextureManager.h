...
public:
    ...
    //     );
    Texture* GetOrCreateTextureFromFile(const std::string& FilePath, ResourceUploader* pUploader);
    Texture* GetOrCreateTexture(FDevice* pDevice, ResourceUploader* pUploader, const std::string& Name, const void* pData, uint32_t Width, uint32_t Height, DXGI_FORMAT Format, uint32_t BytesPerPixel);
    void DestroyAll();
